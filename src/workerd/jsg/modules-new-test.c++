...
KJ_TEST("Attaching a module registry works") {
  ...
  registry->attachToIsolate(lock, compilationObserver);

  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///worker1");
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "worker1");
    KJ_ASSERT(val.isNumber());
  }, [&](Value exception) {
    lock.throwException(kj::mv(exception));
  });
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///worker2");
    KJ_ASSERT(val.isNumber());
  }, [&](Value exception) {
    lock.throwException(kj::mv(exception));
  });
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///main", "m"_kjc);
...
KJ_TEST("Basic types of modules work (text, data, json, wasm)") {
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///worker", "abc"_kjc);
    KJ_ASSERT(val.isString());
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///worker", "xyz"_kjc);
  ...
  lock.tryCatch([&] {
    auto val1 = ModuleRegistry::resolve(lock, "file:///worker", "json"_kjc);
    auto val2 = ModuleRegistry::resolve(lock, "file:///json", "default"_kjc);
    ...
    KJ_ASSERT(obj.get(lock, "foo").isNumber());
  }, [&](Value exception) {
    lock.throwException(kj::mv(exception));
  });

  lock.tryCatch([&] {
    auto wasm1 = ModuleRegistry::resolve(lock, "file:///worker", "wasm"_kjc);
    auto wasm2 = ModuleRegistry::resolve(lock, "file:///wasm", "default"_kjc);
    auto wasm3 = ModuleRegistry::resolve(lock, "file:///worker", "wasm2"_kjc);
...
KJ_TEST("compileEvalFunction in synthetic module works") {
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///main");
...
KJ_TEST("import.meta works as expected") {
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///foo");
    KJ_ASSERT(val.isObject());
    auto obj = KJ_ASSERT_NONNULL(val.tryCast<JsObject>());
    JsValue url = obj.get(lock, "url");
    JsValue main = obj.get(lock, "main");
    JsValue res = obj.get(lock, "resolve");

    KJ_ASSERT(url.isString());
    KJ_ASSERT(main.isBoolean());
    KJ_ASSERT(res.isFunction());

    KJ_ASSERT(url.toString(lock) == "file:///foo"_kj);

    auto mainVal = KJ_ASSERT_NONNULL(main.tryCast<JsBoolean>());
    KJ_ASSERT(!mainVal.value(lock));

    auto& wrapper = TestIsolate_TypeWrapper::from(lock.v8Isolate);
    KJ_IF_SOME(fn, wrapper.tryUnwrap(lock.v8Context(),
                                     res, (Function<kj::String(kj::String)>*)nullptr,
                                     kj::none)) {
      KJ_ASSERT(fn(lock, kj::str("foo/bar")) == "file:///foo/bar"_kj);
    }
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "file:///foo/bar");
    KJ_ASSERT(val.isObject());
    auto obj = KJ_ASSERT_NONNULL(val.tryCast<JsObject>());
    JsValue url = obj.get(lock, "url");
    JsValue main = obj.get(lock, "main");
    ...
    KJ_ASSERT(url.toString(lock) == "file:///foo/bar"_kj);
...
KJ_TEST("import specifiers with query params and hash fragments work") {
  ...
  lock.tryCatch([&] {
    auto val1 = ModuleRegistry::resolve(lock, "file:///foo?1");
    auto val2 = ModuleRegistry::resolve(lock, "file:///foo?2");
    auto val3 = ModuleRegistry::resolve(lock, "file:///foo#1");
    auto val4 = ModuleRegistry::resolve(lock, "file:///foo#2");
...
KJ_TEST("Previously resolved modules not found with incompatible resolve context") {
  ...
  lock.tryCatch([&] {
    ...
    auto value1 = ModuleRegistry::resolve(lock, "foo:bar", "default"_kjc,
    ...
    ModuleRegistry::resolve(lock, "foo:bar", "default"_kjc,
  ...
  lock.tryCatch([&] {
    auto value2 = ModuleRegistry::resolve(lock, "file:///bar", "default"_kjc,
                                          ResolveContext::Type::BUNDLE);
    KJ_ASSERT(value2.isArrayBuffer());

    // But should not be found from a built-in context
    ModuleRegistry::resolve(lock, "file:///bar", "default"_kjc,
                            ResolveContext::Type::BUILTIN);
    JSG_FAIL_REQUIRE(Error, "Should have thrown");
  }, [&](Value exception) {
    auto str = kj::str(exception.getHandle(lock));
...
KJ_TEST("Awaiting top-level dynamic import in synchronous require works") {
  ...
  lock.tryCatch([&] {
    ...
    auto val = ModuleRegistry::resolve(lock, "file:///foo", "default"_kjc);
    KJ_ASSERT(val.isNumber());
  }, [&](Value exception) {
    lock.throwException(kj::mv(exception));
  });
...
KJ_TEST("Throwing an exception inside a ESM module works as expected") {
  ...
  lock.tryCatch([&] {
    ModuleRegistry::resolve(lock, "file:///foo", "default"_kjc);
    JSG_FAIL_REQUIRE(Error, "Should have thrown");
  }, [&](Value exception) {
    auto str = kj::str(exception.getHandle(lock));
...
KJ_TEST("Throwing an exception inside a CJS-style eval module works as expected") {
  ...
  lock.tryCatch([&] {
    ModuleRegistry::resolve(lock, "file:///foo", "foo"_kjc);
    JSG_FAIL_REQUIRE(Error, "Should have thrown");
  }, [&](Value exception) {
    auto str = kj::str(exception.getHandle(lock));
...
KJ_TEST("Recursive import works or fails as expected") {
  ...
  auto val1 = ModuleRegistry::resolve(lock, "file:///foo", "default"_kjc);
...
KJ_TEST("Recursively require ESM from CJS required from ESM fails as expected") {
  ...
  lock.tryCatch([&] {
    ...
    auto str = kj::str(exception.getHandle(lock));
    KJ_ASSERT(str == "TypeError: Circular module dependency with synchronous require: file:///bar");
  });
}
...
KJ_TEST("Resolution occurs relative to the referrer") {
  ...
  lock.tryCatch([&] {
    auto abc = ModuleRegistry::resolve(lock, "file:///foo/", "abc"_kjc);
    auto def = ModuleRegistry::resolve(lock, "file:///foo/", "def"_kjc);
    auto ghi = ModuleRegistry::resolve(lock, "file:///foo/", "ghi"_kjc);
    auto jkl = ModuleRegistry::resolve(lock, "file:///foo/", "jkl"_kjc);
    auto lmn = ModuleRegistry::resolve(lock, "file:///foo/", "lmn"_kjc);
...
KJ_TEST("Building a bundle from a capnp description works") {
  ...
  lock.tryCatch([&] {
    auto val = ModuleRegistry::resolve(lock, "foo:bar");
    KJ_ASSERT(val.isNumber());
  }, [&](Value exception) {
    lock.throwException(kj::mv(exception));
  });
...
KJ_TEST("Percent-encoding in specifiers is normalized properly") {
  ...
  lock.tryCatch([&] {
    auto abc = ModuleRegistry::resolve(lock, "foo", "abc"_kjc);
    auto def = ModuleRegistry::resolve(lock, "foo", "def"_kjc);
    auto ghi = ModuleRegistry::resolve(lock, "foo", "ghi"_kjc);
    auto jkl = ModuleRegistry::resolve(lock, "foo", "jkl"_kjc);
    auto mno = ModuleRegistry::resolve(lock, "foo", "mno"_kjc);
    auto pqr = ModuleRegistry::resolve(lock, "foo", "pqr"_kjc);
...
KJ_TEST("Aliased modules (import maps) work") {
  ...
  lock.tryCatch([&] {
    ...
    auto abc = ModuleRegistry::resolve(lock, "qux", "abc"_kjc);
    auto def = ModuleRegistry::resolve(lock, "qux", "def"_kjc);
...
KJ_TEST("Import attributes are currently unsupported") {
  ...
  lock.tryCatch([&] {
    ModuleRegistry::resolve(lock, "foo", "default"_kjc);
    JSG_FAIL_REQUIRE(Error, "Should have thrown");
  }, [&](Value exception) {
    auto str = kj::str(exception.getHandle(lock));
  ...
  lock.tryCatch([&] {
    ModuleRegistry::resolve(lock, "bar", "default"_kjc);
    JSG_FAIL_REQUIRE(Error, "Should have thrown");
  }, [&](Value exception) {
    auto str = kj::str(exception.getHandle(lock));
    KJ_ASSERT(str == "TypeError: Import attributes are not supported");
