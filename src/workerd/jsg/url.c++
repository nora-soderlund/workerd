...
kj::Array<const char> normalizePathEncoding(kj::ArrayPtr<const char> pathname) {
  ...
  // through ourselves rather than using something like kj::delimited. This is
  // simple enough, tho. We'll percent decode as we go, re-encode the pieces
  // and then join them back together with %2F.

  static constexpr auto findNext = [](std::string_view input)
      -> kj::Maybe<size_t> {
    size_t pos = input.find("%2f", 0);
